...
import glfw  # Import glfw to access key constants
import loguru
import mujoco
...
def main(
    ...
    if "ctrl" in traj_data:
        ...
        loguru.logger.warning("No ctrl in trajectory file, using qpos as ctrl")
        if robot_type == "g1":
    ...
    with run_viewer() as gui:
        ...
        def reset_simulation():
            """Resets the simulation to the first frame."""
            nonlocal cnt
            data.qpos[:] = qpos_list[0]
            data.qvel[:] = qvel_list[0]
            data.ctrl[:] = ctrl_list[0]
            mujoco.mj_forward(model, data)
            cnt = 0
            loguru.logger.info("Simulation has been reset.")

        def key_callback(keycode):
            """Callback for keyboard events."""
            nonlocal is_paused
            # Use glfw constants for keys
        ...
        while gui.is_running():
                ...
                for side in sides:
                    for tip_name in tips:
                        ...
                        hand_sites_id.append(
                            mujoco.mj_name2id(
                                model, mujoco.mjtObj.mjOBJ_SITE, hand_site
                            )
                        )
            ...
            # update reference and sync viewer, even when paused
            if show_viewer:
